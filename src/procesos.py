...
def mostrar_resumen_puntajes(acumulado):
    """
    Args:
        acumulado (dict): Diccionario con los datos acumulados por equipo.
    """
    print("\nResumen de Puntajes Totales por Equipo:")
    resumen = list(map(lambda item: f"{item[0]}: {item[1]['puntos']} puntos", acumulado.items()))
