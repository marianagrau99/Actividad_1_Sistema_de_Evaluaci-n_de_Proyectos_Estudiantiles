def calcular_puntaje(ronda):
    ...
    """
    return {equipo: 3*datos['innovacion'] + 
            1*datos['presentacion'] - 
            (1 if datos['errores'] else 0)
...
def mostrar_tabla(acumulado):
    ...
    Args:
        acumulado (dict): Diccionario con datos acumulados por equipo.
    """
    print(f"{'Equipo':<10} {'Innovación':<10} {'Presentación':<13} {'Errores':<8} {'Mejores Equipos'.center(18)} {'Puntos':<6}")
    
    # Ordenar de forma descendente a los equipos por puntos
    equipos_ordenados = sorted(acumulado.items(), key=lambda x: x[1]['puntos'], reverse=True)
    
    for equipo, datos in equipos_ordenados:
        print(f"{equipo:<10} {datos['innovacion']:<10} {datos['presentacion']:<13} {datos['errores']:<8} {str(datos['mejores']).center(18)} {datos['puntos']:<6}")
